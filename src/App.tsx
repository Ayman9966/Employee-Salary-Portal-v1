...
export default function App() {
  ...
  const [deviceOS, setDeviceOS] = useState<"ios" | "android" | "other">(
    "other",
  );
  ...
  useEffect(() => {
    const company = joinedWorkspace || "AirSlip";
    document.title = `${company} - Employee Portal`;

    const manifest = {
      name: `${company} Employee Portal`,
      short_name: company,
      start_url: window.location.origin + window.location.pathname,
      display: "standalone",
      background_color: "#f9f9ff",
      theme_color: "#003d9b",
      description: `Securely access your salary slips from ${company}.`,
      icons: [
        {
          src: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23003d9b'%3E%3Cpath d='M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z'/%3E%3C/svg%3E",
          sizes: "192x192",
          type: "image/svg+xml",
        },
        {
    ...
    if (!appleIcon) {
      ...
      appleIcon.setAttribute(
        "href",
        "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23003d9b'%3E%3Cpath d='M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z'/%3E%3C/svg%3E",
      );
  ...
  // Handle automatic PWA Install Prompt
      ...
      try {
        ...
        if (decoded && decoded.gasUrl) {
          ...
          if (decoded.companyName)
            localStorage.setItem("company_name", decoded.companyName.trim());
          if (decoded.email)
            localStorage.setItem("company_email", decoded.email.trim());
          if (decoded.whatsapp)
            localStorage.setItem("company_whatsapp", decoded.whatsapp.trim());
          if (decoded.companySize)
            localStorage.setItem("company_size", decoded.companySize.trim());
        ...
        console.error(
          "Failed to parse dynamic onboarding invitation token:",
          err,
        );
      }
    }
  ...
  const [currentView, setCurrentView] = useState<
    "profile" | "history" | "documents" | "admin"
  >("history");
  ...
  const [summary, setSummary] = useState<DashboardSummary>(
    getFromCache<DashboardSummary>(`dashboard_${new Date().getFullYear()}`) || {
      annualNet: 0,
      availableYears: [],
    },
  );
  ...
  const [lang, setLang] = useState<"en" | "ar">(
    () => (localStorage.getItem("app_lang") as "en" | "ar") || "en",
  );
  ...
  useEffect(() => {
    if (accessCode) {
      ...
      Promise.all([fetchSalarySlips(), fetchDashboardSummary(selectedYear)])
        .then(([freshSlips, freshSummary]) => {
          ...
          if (freshSlips.length > 0 && !selectedSlip) {
            setSelectedSlip({
              month: freshSlips[0].month,
              year: freshSlips[0].year,
            });
          }
        })
        .catch((err) => {
          ...
          setError(
            "Connection failed. Please check your VITE_GAS_URL or internet connection.",
          );
          setIsLoading(false);
          setIsSyncing(false);
        });
      ...
      <div className="min-h-screen bg-rose-50/50 flex items-center justify-center p-6 antialiased font-sans text-rose-950">
        <div className="bg-white p-8 rounded-3xl shadow-xl shadow-rose-100 max-w-md w-full border border-rose-100/80 text-center space-y-5 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="space-y-2">
            <h1 className="text-xl font-bold text-rose-950 tracking-tight">
              Workspace Suspended
            </h1>
            <p className="text-xs text-rose-600 leading-relaxed font-semibold">
              This payroll workspace has been suspended by the super
              administrator. Access to records, pay slips, and admin management
          ...
          <div className="bg-rose-50/40 p-4 rounded-2xl border border-rose-100/50 font-medium text-[11px] text-slate-500 leading-normal">
            Please contact your organization's human resource administrative
            manager or system integrator for billing or access updates.
          </div>
        </div>
      </div>
    );
    ...
    <>
      ...
      {inviteCountdown !== null && !isStandalone && (
        <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 via-[#0b251e] to-slate-900 text-white border-b border-emerald-500/10 z-[100] animate-in slide-in-from-top duration-300">
          ...
          <div
            className="h-0.5 bg-gradient-to-r from-emerald-500 via-teal-400 to-emerald-500 transition-all duration-1000 ease-linear"
            style={{ width: `${(inviteCountdown / 60) * 100}%` }}
          />
        </div>
      )}
      ...
      {!accessCode ? (
              ...
              <div>
                <div className="text-center mb-6">
                  ...
                  <p className="text-slate-500 text-sm mt-1">
                    Sign in with management credentials
                  </p>
                </div>
        ...
        >
          ...
          />
          <main
            className={`${currentView === "admin" ? "max-w-7xl px-4 md:px-6" : "max-w-md px-3"} mx-auto py-4`}
          >
                          ...
                          <div className="bg-white rounded-xl border border-[#D1E1F5] overflow-hidden shadow-sm p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                              ...
                              >
                                ...
                                <span>
                                  {lang === "en"
                                    ? "Prepare Setup URL (60s)"
                                    : "رابط التثبيت (60 ثانية)"}
                                </span>
                              </button>
                            </div>
                          </div>
                      ...
                      <div className="space-y-6">
                        <section>
                          <div className="flex justify-between items-end mb-4 gap-4">
                            <div>
                              <span className="text-caption font-bold text-secondary uppercase tracking-wider block mb-1">
                                {t.statementYear}
                              </span>
                              {summary.availableYears.length > 1 ? (
                                <div className="flex flex-col gap-2">
                                  <div className="relative flex items-center">
                                    ...
                                    <span className="material-symbols-outlined absolute right-3 pointer-events-none text-secondary">
                                      expand_more
                                    </span>
                                  </div>
                            ...
                            >
                              <span className="text-caption font-bold text-secondary uppercase tracking-wider block mb-0.5">
                                {t.annualNet}
                              </span>
                              <p className="font-headline-md text-display font-semibold text-primary">
                                {formatAmount(summary.annualNet, {
                        ...
                        {error && (
                          <div className="bg-error-container/10 border border-error/20 p-4 rounded-xl text-center">
                            ...
                            <p className="text-error text-body font-medium">
                              {error}
                            </p>
                            <button
                  ...
                  return (
                    <AdminPanel
                      onSyncTrigger={() => {
                        ...
                        fetchDashboardSummary(selectedYear).then(
                          (freshSumm) => {
            ...
            >
              <div className="text-center md:text-left space-y-2">
                ...
                <p className="text-slate-500 text-xs leading-relaxed">
                  Choose how you want to sign out. You can sign out of your
                  current session, or completely reset this device's company
                  configurations.
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="space-y-0.5 flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 leading-normal">
                      Logs you out but keeps the Google Sheets connection
                      settings.
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="space-y-0.5 flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 leading-normal">
                      Removes session, company details, and Google Apps Script
                      endpoint. Resets system.
                    </p>
                  </div>
                </button>
              </div>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-semibold mt-1">
                      {lang === "en"
                        ? "Setup AirSlip on Android & iOS devices"
                        : "إعداد وتثبيت التطبيق على هواتف أندرويد وآيفون"}
                    </p>
                  </div>
                </div>
                <button
